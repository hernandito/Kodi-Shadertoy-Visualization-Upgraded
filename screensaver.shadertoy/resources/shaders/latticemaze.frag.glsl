...
// Circle mask radius control
#define MASK_RADIUS 2.78

// Reflectivity control
#define REFLECTIVITY 0.7

// Noise control
#define NOISE_SCALE 6.0  // Controls the frequency of the noise (smaller = finer, larger = coarser)
#define NOISE_STRENGTH 1.0  // Controls the intensity of the noise (0.0 = no noise, 1.0 = maximum current strength)

// Swirl pattern control
#define SWIRL_DENSITY 12.0  // Controls the density of swirly patterns (higher = more swirls, smaller size)
...
float hexTruchet(in vec2 p) { 
    ...
    c = fract(cos(mod(dot(h, vec2(41.13, 289.57)), 6.2831))*43758.5453);
    p -= p*step(c, .5)*2.;
    p -= vec2(-1, 0);
    c = dot(p, p);
    p -= vec2(1.5, .8660254);
...
vec3 texFaces(in vec3 p, in vec3 n){
    n = abs(n);
    p.xy = n.x>.5? p.yz : n.y>.5? p.xz : p.xy; 
    n = mix(vec3(.3, .1, .02), vec3(.35, .5, .65), n2D(p.xy*NOISE_SCALE)*NOISE_STRENGTH);
    n *= n2D(p.xy*NOISE_SCALE*2.)*NOISE_STRENGTH*0.5 + 1.0;
    vec2 sc = cos(mod(hexTruchet(p.xy)*6.2831*vec2(2, 4), 6.2831));
...
vec3 texEdges(in vec3 p, in vec3 n){
    float bf = bumpFunc2(p, n);
    n = abs(n);
    p.xy = n.x>.5? p.yz : n.y>.5? p.xz : p.xy; 
    n = mix(vec3(.3, .1, .02), vec3(.35, .5, .65), n2D(p.xy*NOISE_SCALE)*NOISE_STRENGTH);
    n *= n2D(p.xy*NOISE_SCALE*2.)*NOISE_STRENGTH*0.25 + 0.75; 
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float atten = 1.25/(1.0 + lDist*0.1 + lDist*lDist*.05);
    float rt = refTrace(sp + sn*.005, ref);
    float rEdge;
    vec3 rsp = sp + ref*rt;
    vec3 rsn = normal(rsp, rEdge);
    vec3 rCol = texFaces(rsp, rsn);    
    if(rEdge>.001) rCol = texEdges(rsp, rsn);
    float rDiff = max(dot(rsn, normalize(lp-rsp)), 0.);
    float rSpec = pow(max(dot(reflect(-normalize(lp-rsp), rsn), -ref), 0.), 8.);
    float rlDist = length(lp - rsp);
    rCol = (rCol*(rDiff*1. + vec3(.45, .4, .3)) + vec3(1., .6, .2)*rSpec*2.);
    rCol *= 1.25/(1.0 + rlDist*0.1 + rlDist*rlDist*.05);    
    col = oCol*(diff*1. + vec3(.45, .4, .3)) + vec3(1., .6, .2)*spec*2. * REFLECTIVITY;
    if(edge<.001) col += rCol*.2 * REFLECTIVITY;
    else col += rCol*.35 * REFLECTIVITY;
    col = min(col*atten*sh*ao, 1.);
    vec3 bg = mix(vec3(.5, .7, 1).zyx, vec3(1, .7, .3).zyx, -rd.y*.35 + .35);
    col = mix(col, bg, smoothstep(0., FAR-25., t));
    ...
    // Apply circular mask with adjustable radius
    vec2 center = vec2(0.5, 0.5); // Center of the screen
    float aspect = iResolution.x / iResolution.y; // Aspect ratio correction
    vec2 uvAdjusted = uv - center; // Shift to center
    uvAdjusted.x *= aspect; // Scale x by aspect to make circle uniform
    float dist = length(uvAdjusted);
    float radius = MASK_RADIUS; // Use defined radius
    if (dist > radius) {
        col = vec3(0.0); // Black background outside the circle
    }
