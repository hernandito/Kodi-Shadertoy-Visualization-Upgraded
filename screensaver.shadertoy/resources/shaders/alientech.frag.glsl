...
vec3 light(vec2 p, vec3 col) {
	...
	// calculate normals based on horizontal and vertical vectors being z the formula result iTime
	vec2 d = vec2(0., .003);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	// yellow lights
	col+= pow(lig * .12, 15.) * vec3(1.,.6,.1) * (.8 + BLINKINGLIGHTS * sin(ti * 10. - uv.y * 10.) * .6);
