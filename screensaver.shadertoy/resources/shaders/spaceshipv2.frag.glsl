...
void mainImage(out vec4 O, vec2 I)
{
    ...
    // Tunable parameters
    float zoomFactor = 90.; // Controls the zoom level of the effect
    ...
    float particleSpeed = 0.008; // Controls the speed of the particles (affects trail length)
    ...
    float turbulenceMapScale = 2.0; // Controls the scale of the smoke turbulence (cloud noise size)
    ...
    // Construct rotation matrix dynamically with scaling factor 5 (magnitude of original mat2(3, 4, 4, -3))
    ...
    for(float t = iTime, T = .1 * t + p.y, i = 0.; i++ < 50.;
    ...
    // Render the particle effect with black background
    O = clamp(S * S, 0., 1.);
...
/** SHADERDATA
{
    ...
    "description": "Simulates glowing particle trails inspired by SpaceX Starship debris",
    "model": "person"
}
*/