...
// --- GLOBAL PARAMETERS ---
#define ANIMATION_SPEED .075 // Controls the overall animation speed (1.0 = normal speed)
#define FOV_ANGLE 0.9       // Field of View angle (e.g., 1.0 for normal, smaller for zoom in)
#define REFLECTION_STRENGTH 0.185 // Controls the amount of reflectivity for iChannel0 textured geometry (0.0 = no reflection, 1.0 = full reflection)

// --- POST-PROCESSING DEFINES (BCS) ---
#define BRIGHTNESS 1.0    // Brightness adjustment (1.0 = neutral)
#define CONTRAST 1.0      // Contrast adjustment (1.0 = neutral)
...
vec3 tex3D( sampler2D tex, in vec3 p, in vec3 n ){
    ...
    n /= max((n.x + n.y + n.z), 1e-6); // Robust division
...
float sminP( float a, float b, float smoothing ){
    ...
    float h = clamp( 0.5+0.5*(b-a)/max(smoothing, 1e-6), 0.0, 1.0 ); // Robust division
...
float calculateAO(in vec3 p, in vec3 n){
    ...
    for(float i=1.; i<nbIte+.5; i++){
        ...
        l = (i + hash(i))*.5/max(nbIte, 1e-6)*maxDist; // Robust division
        ao += (l - map( p + n*l ))/ max(pow(1. + l, falloff), 1e-6); // Robust division
    }
    
    return clamp( 1.-ao/max(nbIte, 1e-6), 0., 1.); // Robust division
}
...
float softShadow(vec3 ro, vec3 lp, float k){
    ...
    float stepDist = end/max(float(maxIterationsShad), 1e-6); // Robust division
    
    rd /= max(end, 1e-6); // Robust division
    ...
    for (int i=0; i<maxIterationsShad; i++){
        ...
        shade = min(shade, smoothstep(0.0, 1.0, k*h/max(dist, 1e-6))); // Robust division
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    vec3 rd_unnormalized = (vec3(2.*fragCoord - iResolution.xy, iResolution.y));
    // Apply FOV_ANGLE scaling
    rd_unnormalized.xy /= FOV_ANGLE;
    ...
    vec3 rd = normalize(vec3(rd_unnormalized.xy, sqrt(max(rd_unnormalized.z*rd_unnormalized.z - dot(rd_unnormalized.xy, rd_unnormalized.xy)*.2, 0.))));
    
    // Rotating the ray with Fabrice's cost cuttting matrix. I'm still pretty happy with this also. :)
    vec2 m = sin(vec2(0, 1.57079632) + iTime * ANIMATION_SPEED / 4.); // Use ANIMATION_SPEED
    rd.xy = mat2(m.y, -m.x, m)*rd.xy;
    rd.xz = mat2(m.y, -m.x, m)*rd.xz;
    
    
    // Ray origin, set off in the Z direction.
    vec3 ro = vec3(0.0, 0.0, iTime * ANIMATION_SPEED); // Use ANIMATION_SPEED
    ...
    if(t<FAR){
        ...
        ld /= max(lDist, 1e-6); // Normalizing the light direction vector, robust division
        ...
        float atten = 1.0 / max((1.0 + lDist*0.25 + lDist*lDist*.1), 1e-6); // Attenuation, robust division
        ...
        float rAtten = 1./max((1.0 + rlDist*0.25 + rlDist*rlDist*.1), 1e-6); // Robust division
        ...
        col = oCol*(diff + vec3(.5, .6, .7)) + vec3(.5, .7, 1)*spec*2. + rCol*REFLECTION_STRENGTH; // Use REFLECTION_STRENGTH
