...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // Rotation matrix for diagonal motion (upper left to lower right)
    float c = 13.0 / sqrt(338.0); // cos(theta)
    float s = 10.0 / sqrt(338.0); // sin(theta)
    ...
    for (; i < 15.0; i++)
    {
        ...
        O += color * glowIntensity / length(max(p, p / vec2(turbulence * 20.0, 2)));
        // Fine-tune width: decrease (e.g., 15.0) for wider fronts/glow, increase (e.g., 25.0) for thinner
        // Fine-tune color: adjust 0.7 blue (0.6 warmer, 0.8 cooler), 1.2 red (1.3 more red, 1.1 less)
        
        // Shift position for spacing and trail length with front variation, added t * 0.05 for continuous motion
        ...
        // Fine-tune trail length: p.x * 0.000005; decrease (e.g., 0.000003) for longer, increase (e.g., 0.00001) for shorter
