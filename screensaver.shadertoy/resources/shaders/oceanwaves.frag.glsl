...
/*
    ...
    Author:      Tobias Fremming (@tobiasfremming)
    Created:     2025-07-08
    License:     MIT
    Repository:  https://github.com/tobiasfremming/Shader  

    Description:
      A real-time ocean surface simulation using multi-directional Gerstner waves,
      layered noise, and Fresnel reflection.

    Usage:
      • Designed for Shadertoy or as a GLSL fragment shader in OpenGL.
      • Pass in iTime, iResolution, iMouse if you want to use it outside of shadertoy.
      • Tweak wave parameters, light position, and color constants for your scene.

    Attribution:
      This shader is released under the MIT License.
      If you build upon or use this code, a shout-out or link back is greatly appreciated!
...
#define CONTRAST 1.1      // Contrast adjustment (1.0 = neutral)
#define SATURATION 1.2    // Saturation adjustment (1.0 = neutral)
...
#define FIELD_OF_VIEW_MULTIPLIER 1.2 // Default to 1.0 for no scaling
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // --- BCS ADJUSTMENT ---
    vec3 finalColor = fragColor.rgb; // Start with the processed color
    float luminance = dot(finalColor, vec3(0.299, 0.587, 0.114));
    vec3 saturated = mix(vec3(luminance), finalColor, SATURATION);
    vec3 contrasted = (saturated - 0.5) * CONTRAST + 0.5;
    fragColor.rgb = clamp(contrasted + (BRIGHTNESS - 1.0), 0.0, 1.0);
