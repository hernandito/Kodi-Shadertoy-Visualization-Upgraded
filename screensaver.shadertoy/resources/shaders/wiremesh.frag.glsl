...
#define ANIM_SPEED_FACTOR .2

// Post-Processing BCS (Brightness, Contrast, Saturation) iTime
...
vec3 envMap(vec3 p){
    p *= 2.;
    p.xz += iTime*ANIM_SPEED_FACTOR*.5;
    float n3D2 = n3D(p*2.);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    vec3 lk = vec3(0, -.5 - .125*.5, iTime*ANIM_SPEED_FACTOR + 1.);
    ...
    if(t < FAR){
        ...
        if(svObjID>0.5) {
            ...
            texCol = smoothstep(0.0, .5, texCol);
    ...
    sceneCol += POST_BRIGHTNESS;
    sceneCol = (sceneCol - 0.5) * POST_CONTRAST + 0.5;
    vec3 luminance = vec3(dot(sceneCol, vec3(0.2126, 0.7152, 0.0722)));
    sceneCol = mix(luminance, sceneCol, POST_SATURATION);
