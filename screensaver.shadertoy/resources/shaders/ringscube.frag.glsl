...
// These control the duration and timing of the different visual phases.
#define CUBE_AND_RING_ANIMATION_START 0.0
#define CUBE_AND_RING_ANIMATION_DURATION 0.5 // Shortened from 1.0
#define FRACTAL_MORPHING_PHASE1_START 1.00    // Starts earlier
#define FRACTAL_MORPHING_PHASE1_DURATION 2.0 // Stretched from 1.0
#define FRACTAL_MORPHING_PHASE2_START 12.0   // Starts earlier
#define FRACTAL_MORPHING_PHASE2_DURATION 2.0 // Stretched from 1.0
#define CAMERA_MOVEMENT_SPEED 0.01            // Controls the camera's orbit speed
#define GROUND_PLANE_SPEED 0.03               // New variable to control the ground plane grid's movement speed
// --- End new variables ---
...
vec3 bb_render(vec3 ro, vec3 rd) {
  ...
  for (int rc = 0; rc < BB_MAX_REFLECTIONS; ++rc) {
      ...
      // Use the new GROUND_PLANE_SPEED variable
      pp1.x -= -2.0*TIME*GROUND_PLANE_SPEED;
      pp1 *= sqrt(0.5);
      vec2 np1 = mod2_1(pp1);
...
vec3 bb_effect(vec2 p, vec2 q) {
  ...
  // Original geometry rotation speed
  bb_g_rot = ROT(TAU*TIME*0.1/4.0);

  // --- Modified animation timing using the new define variables ---
  bb_g_fi = smoothstep(CUBE_AND_RING_ANIMATION_START, CUBE_AND_RING_ANIMATION_DURATION, ltime);
  bb_g_fo = 1.0-smoothstep(bb_period-1.25, bb_period-0.25, ltime);
  bb_g_fi13 = smoothstep(FRACTAL_MORPHING_PHASE1_START, FRACTAL_MORPHING_PHASE1_START + FRACTAL_MORPHING_PHASE1_DURATION, ltime);
  bb_g_fi23 = smoothstep(FRACTAL_MORPHING_PHASE2_START, FRACTAL_MORPHING_PHASE2_START + FRACTAL_MORPHING_PHASE2_DURATION, ltime);
  // --- End modified section ---

  vec3 ro = 0.6*vec3(6.0, 5.0, -2.0);
  vec3 up = vec3(0.0, 1.0, 0.0);

  // Use the new CAMERA_MOVEMENT_SPEED variable
  ro.xz *= ROT(sin(TIME * sqrt(CAMERA_MOVEMENT_SPEED)));
