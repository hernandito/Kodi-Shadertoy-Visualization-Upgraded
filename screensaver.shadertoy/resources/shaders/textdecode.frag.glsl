...
#define STATIC_PASSWORD_LENGTH    18      // "DECODING PASSWORD:"
#define SCRAMBLING_MESSAGE_LENGTH 12      // "I_love_Staci"

#define DISPLAY_HOLD_TIME       10.0    // How long (in seconds) the final message stays on screen after revealing (applies to scrambling lines)
...
#define LINE1_COLOR vec3(0.122, 0.631, 0.122) // Terminal Green for Line 1 ("DECODING PASSWORD:")
...
int getAsciiForStaticPassword(int index) {
    if (index == 0) return 68; // D
    if (index == 1) return 69; // E
    if (index == 2) return 67; // C
    if (index == 3) return 79; // O
    if (index == 4) return 68; // D
    if (index == 5) return 73; // I
    if (index == 6) return 78; // N
    if (index == 7) return 71; // G
    if (index == 8) return 32; // [SPACE]
    if (index == 9) return 80; // P
    if (index == 10) return 65; // A
    if (index == 11) return 83; // S
    if (index == 12) return 83; // S
    if (index == 13) return 87; // W
    if (index == 14) return 79; // O
    if (index == 15) return 82; // R
    if (index == 16) return 68; // D
    if (index == 17) return 58; // :
...
int getAsciiForScramblingMessage(int index) {
    ...
    if (index == 3) return 111; // o
    if (index == 4) return 118; // v
