...
// hex‐grid wrap, returns offset to snapped cell center
...
float ccell2(vec2 p) {
  ...
  cp0 -= vec2(off, 0.0);
  rot(cp0,    TAU*nm/2.0 - iTime*TAU/count);
  ...
  rot(tp2, ctime);
  tp2.x -= 0.065;
  float d6 = box(tp2, vec2(0.0625, 0.0075));
...
// simple ES1.00 round
vec2 round2(vec2 v) { return floor(v + vec2(0.5)); }

float df(in vec2 p, in float scale, inout vec2 nn) {
  p /= scale;
  nn = hextile(p);
  nn = round2(nn);               // <— replaces round(nn)
  float r = hash(nn);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  vec3 col = vec3(160.0)/255.0;
  vec3 baseCol = vec3(0.3);
  float mask = smoothstep(-aa, 0.0, -d);
  col = mix(col, baseCol, pow(mask,8.0));
