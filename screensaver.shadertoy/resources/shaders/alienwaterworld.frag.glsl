// New tanh_approx function for Kodi compatibility
float tanh_approx(float x) {
    const float EPSILON = 1e-6;
    return x / (1.0 + max(abs(x), EPSILON));
}

// === NEW BRIGHTNESS/CONTRAST/SATURATION CONTROLS ===
// BRIGHTNESS: -1.0 (darkest) to 1.0 (brightest). Default is 0.0.
#define BRIGHTNESS -0.10
// CONTRAST: 0.0 (no contrast) to 2.0+ (high contrast). Default is 1.0.
#define CONTRAST 1.3
// SATURATION: 0.0 (grayscale) to 2.0+ (high saturation). Default is 1.0.
...
vec3 postProcess(in vec3 col, in vec2 q)  
{
  ...
  // 2. Apply contrast (pivot around 0.5)
  ...
  col = mix(grayscale, col, SATURATION);

  // 4. Existing post-processing effects
