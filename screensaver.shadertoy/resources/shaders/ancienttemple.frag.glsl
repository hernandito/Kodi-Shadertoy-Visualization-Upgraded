...
#define BRIGHTNESS .3    // Default brightness (1.0 is no change)
#define CONTRAST 1.8      // Default contrast (1.0 is no change)
#define SATURATION 1.0    // Default saturation (1.0 is no change)
#define GAMMA .8         // Default gamma value (2.2 is typical for sRGB)

// ANIMATION SPEED PARAMETER
#define ANIMATION_SPEED .250  // Default animation speed (1.0 is normal speed)

// NOISE SCALE PARAMETER
#define NOISE_SCALE 4.0  // Default noise scale (20.0 is the original scale)

const int Iterations=14;
const float detail=.00001;
const float Scale=2.;
...
float kset(vec3 p) {
	p=abs(.5-fract(p*NOISE_SCALE));  // Changed from 20. to NOISE_SCALE
	float es, l=es=0.;
	for (int i=0;i<13;i++) {
		float pl=l;
		l=length(p);
...
{
	float t=iTime * ANIMATION_SPEED;  // Adjusted time with animation speed
...
{
	tt=iTime * ANIMATION_SPEED;  // Adjusted time with animation speed
    vec2 uv = fragCoord.xy / iResolution.xy*2.-1.;
	uv.y*=iResolution.y/iResolution.x;
	vec2 mouse=(iMouse.xy/iResolution.xy-.5);
	float t=iTime * ANIMATION_SPEED * 0.15;  // Adjusted time with animation speed
	float y=(cos(iTime * ANIMATION_SPEED * 0.1 + 3.)+1.);
	if (iMouse.z<1.) mouse=vec2(sin(t*2.),cos(t)+.3)*.15*(.5+y)*min(1.,iTime*.1);
	uv+=mouse*1.5;
	uv.y-=.1;
	//uv+=(texture(iChannel1,vec2(iTime*.15)).xy-.5)*max(0.,h)*7.;
	vec3 from=vec3(0.0,3.04+y*.1,-2.+iTime * ANIMATION_SPEED * 0.05);  // Adjusted time with animation speed
    ...
    color = mix(vec3(luminance), color, SATURATION);  // Saturation
