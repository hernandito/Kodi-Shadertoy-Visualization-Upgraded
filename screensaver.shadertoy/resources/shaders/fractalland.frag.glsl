...
// There are no lights and no AO, only color by normals and dark edges. fov 1.5
...
// Original parameters for core shader behavior
#define BRIGHTNESS .8
#define GAMMA 1.7
#define SATURATION .0

// New Post-Processing Parameter for Contrast
#define POST_CONTRAST 1.0     // Contrast (1.0 for no change, >1.0 for more contrast)
...

{
    ...
    float an=atan(dir.x,dir.y)+iTime*0.5; // angle for drawing and rotating sun
    ...
    // Original Gamma, Brightness, and Saturation applied here
    col=pow(col,vec3(GAMMA))*BRIGHTNESS;
    col=mix(vec3(length(col)),col,SATURATION);
#ifdef SHOWONLYEDGES
    col=1.-vec3(length(col));
#else
    col*=vec3(1.,.9,.85); // This is a base tint, not part of BCS
#ifdef NYAN
    dir.yx*=rot(dir.x);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Apply only Contrast post-processing
    // Contrast (adjust around 0.5 gray)
    color = mix(vec3(0.5), color, POST_CONTRAST);
