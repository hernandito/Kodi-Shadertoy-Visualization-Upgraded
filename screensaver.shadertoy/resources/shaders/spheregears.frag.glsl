...
// sell this Work and you cannot mint an NFTs of it or train a neural  iTime
...
// PART 1: http://www.youtube.com/watch?v=sl9x19EnKng
// PART 2: http://www.youtube.com/watch?v=bdICU2uvOdU
//
// Video capture here: http://www.youtube.com/watch?v=ydTVmDBSGYQ
//
...
vec3 calcNormal( in vec3 pos, in float time )
{
    ...
    const float eps = 0.0005; // Adjusted to match original intent
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for( int n=ZERO; n<AA; n++ )
    {
        ...
        float time = iTime*.1 - 0.5*(1.0/24.0)*(float(m*AA+n)+d)/float(AA*AA-1);
        #else
