// CC0: Recursion for Windows Terminal
...
// Adjustable parameter for hue shift of the diffuse color
const float hueShift = -0.65; // Adjust to shift diffuse color hue: 0.0 is default (magenta), -0.65 shifts to amber (e.g., 0.1 hue), positive values shift toward blue
...
const vec3 diffCol    = HSV2RGB(vec3(hoff+0.75+hueShift, 0.85, 1.0)); // Adjusted hue to shift from magenta to amber
const vec3 sunCol1    = HSV2RGB(vec3(hoff+0.60, 0.50, 0.5));
const vec3 sunCol2    = HSV2RGB(vec3(hoff+0.05, 0.75, 1.0));
