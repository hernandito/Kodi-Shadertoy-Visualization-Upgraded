...
// Redefine ZERO macro to avoid using iFrame, which is unavailable in Kodi Shadertoy addon
#define ZERO 0
...
vec4 map( in vec3 pos, float atime )
{
    ...
    float rc = cos(rot);
    float rs = sin(rot);
    q.xy = mat2(rc,rs,-rs,rc)*q.xy; // Replaced mat2x2 with mat2
    vec3 r = q;
    href = q.y;
    ...
    h.xz = mat2(cos(hr),sin(hr),-sin(hr),cos(hr))*h.xz; // Replaced mat2x2 with mat2
    ...
    cq.xy = mat2(0.8,0.6,-0.6,0.8)*cq.xy; // Replaced mat2x2 with mat2
...
{
    // Use the simpler implementation to avoid bitwise operations, which are not supported in GLSL ES 1.00
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // - Default value: 0.35 (slower than original speed)
    // - Range: 0.0 to infinity
    // - 1.0 = original speed (as defined by the original time scaling of 0.9)
    // - Values < 1.0 slow down the animation (e.g., 0.5 = half speed)
    // - Values > 1.0 speed up the animation (e.g., 2.0 = double speed)
    // - Example: To make the animation 25% slower than the original, set to 0.75 (since 0.9 * 0.75 = 0.675)
    float animationSpeed = 0.35;

    vec3 tot = vec3(0.0);
    vec2 p = (-iResolution.xy + 2.0*fragCoord)/iResolution.y;
    float time = iTime;
    time += -2.6;
    time *= 0.9 * animationSpeed; // Apply animation speed scaling
    ...
    // Adjustable parameter for field of view (FOV)
    // focalLength: Controls the camera's field of view.
    // - Default value: 1.8 (original FOV)
    // - Range: 0.1 to 5.0 (practical range for visible results)
    // - Smaller values widen the FOV (e.g., 1.5 for a slightly wider shot)
    // - Larger values narrow the FOV (e.g., 2.5 for a zoomed-in shot)
    // - Example: To widen the shot by about 15%, set to 1.5
    // Note: Very small values (e.g., < 0.5) may cause distortion, while very large values (e.g., > 5.0) may make the scene appear too zoomed in.
    float focalLength = 1.1;

    // ray direction with adjustable FOV
    vec3 rd = ca * normalize( vec3(p, focalLength) );
