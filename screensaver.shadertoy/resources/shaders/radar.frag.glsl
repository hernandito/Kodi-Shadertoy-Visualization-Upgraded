...
vec3 radar(vec2 p,vec3 col){
    ...
    col = mix(col,vec3(0.286, 0.639, 0.286)*a*0.17,S(d,0.0));
    
    d = length(p)-0.4;
    a = radians(1.);
    p.x = abs(p.x);
    d = max(dot(p,vec2(cos(a),sin(a))),d);
    col = mix(col,vec3(0.286, 0.639, 0.286),S(d,0.0));
...
vec3 objects(vec2 p, vec3 col){   // These are the little space invaders in the center scope
    vec2 prevP = p;
    p.y+= iTime*0.1;
    p*=5.;
    vec2 id = floor(p);
    ...
    col = mix(col,vec3(1,0,0),S(bd,0.0));
    return col;
}

vec3 graph0(vec2 p, vec3 col){   // This is what looks like an audio graph top left.
    p*=1.3;
    vec2 prevP = p;
    p.x+=iTime*0.2;
    p*=120.;
...
vec3 graph1(vec2 p, vec3 col){          //This looks like bar chart lower left
    p*=1.3;
    vec2 prevP = p;
    p.y+=0.11;
    p.x+=-iTime*0.1;
...
vec3 graph2(vec2 p, vec3 col){    // This is the sine wave on top right
    p*=1.3;
    vec2 prevP = p;
    p*=15.;
    p.x+=iTime*1.5;
...
vec3 graph3(vec2 p, vec3 col){      //Guitar bar codes. 
    p*=1.3;
    vec2 prevP = p;
...
vec3 smallCircleUI(vec2 p, vec3 col){    //  SMALL RADAR SCREEN
    vec2 prevP = p;
    ...
    float a = atan(p.x,p.y);
    d = length(p)-0.1;
    col = mix(col,vec3(0.286, 0.639, 0.286)*a*0.25,S(d,0.0));  // Transparency scan gradient
...
vec3 smallCircleUI2(vec2 p, vec3 col){    //   COUNTDOWN Clock
    vec2 prevP = p;
    ...
    d = min(d,d2);
    col = mix(col,vec3(.5),S(d,0.0));
    
    p = prevP;
    d = abs(length(p)-0.1)-0.01;
    col = mix(col,vec3(.4),S(d,0.0));
    ...
    col = mix(col,vec3(.6),S(d,0.0));
    
    return col;
}

vec3 smallCircleUI3(vec2 p, vec3 col, float dir){		//  Small elevation marcks
    vec2 prevP = p;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Post-processing: Apply lighten blend to replace blacks with dark green
    vec3 darkGreen = vec3(0.0, 0.05, 0.0);
    col = max(col,darkGreen);
    
    // Post-processing: Apply darken blend to tint whites and grays to terminal green
    vec3 terminalGreen=vec3(0.286, 0.639, 0.286);
    col = min(col,terminalGreen);
