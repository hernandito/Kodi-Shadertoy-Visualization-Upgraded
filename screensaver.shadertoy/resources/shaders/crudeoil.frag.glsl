...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // --- Light Color ---
  // Original value: vec3(1.0, 0.8, 0.6) (light peachy tone)
  // Adjusted to warmer tone: vec3(1.0, 0.7, 0.4) (more red, less blue)
  vec3 baseCol = vec3(1.0, 0.7, 0.4);

  float oh = height(p + ll1 * 0.05 * normalize(ld1.xz));
  const float level0 = 0.0;
  const float level1 = 0.125;
  // VERY VERY fake shadows + highlight
