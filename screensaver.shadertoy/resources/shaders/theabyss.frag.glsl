...
// Initialize global variables to avoid uninitialized warnings
mat2 g_rot = mat2(1.0, 0.0, 0.0, 1.0); // Identity matrix
float g_scale = 1.85; // Default value from effect
...
float df(vec3 p, float w, out float off) {
  vec4 p4 = vec4(p, 0.1);
  p4.yw *= g_rot;
  p4.zw *= mat2(g_rot[0][0], g_rot[1][0], g_rot[0][1], g_rot[1][1]);
...
// Post-processing function for brightness, contrast, saturation
vec3 applyPostProcessing(vec3 color, float brightness, float contrast, float saturation) {
    color += brightness;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // Apply post-processing with adjusted default brightness
  col = applyPostProcessing(col, -0.2, 0.95, 0.90); // Changed brightness to 0.2 to allow darkening
  // col = applyPostProcessing(col, 1.0, 1.0, 1.0); // Neutral settings (no change)
