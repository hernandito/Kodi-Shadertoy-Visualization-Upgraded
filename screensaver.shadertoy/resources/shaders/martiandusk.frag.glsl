...
// --- GLOBAL PARAMETERS ---
#define GLOBAL_ANIMATION_SPEED 0.20 // Adjusts the overall speed of the animation (1.0 = normal speed)

// Primary Color Parameters
#define HORIZON_BASE_COLOR vec3(0.702, 0.239, 0.016) // Base color for the horizon/sky gradient (default reddish-orange)
#define BOUNCED_LIGHT_COLOR vec3(0.8, 0.38, 0.106) // Color for bounced light/ground influence (default orange-red)

// Post-processing BCS Parameters
#define POST_BRIGHTNESS 0.0      // Brightness adjustment (-1.0 to 1.0, 0.0 = no change)
#define POST_CONTRAST   1.00      // Contrast adjustment (0.0 to inf, 1.0 = no change)
#define POST_SATURATION 1.0      // Saturation adjustment (0.0 to inf, 1.0 = no change)
#define POST_CLAMP_BRIGHTNESS_OUTPUT 1.0 // Clamps the brightest output color (1.0 = no clamp, <1.0 clamps values)
...
void mainImage(out vec4 frag_col, in vec2 frag_coord) {
  ...
  // Apply GLOBAL_ANIMATION_SPEED
  float z = (iTime * GLOBAL_ANIMATION_SPEED * 1.0);
  vec2 sc = sincos((iTime * GLOBAL_ANIMATION_SPEED * 0.5));
  ...
  // Use defined horizon and bounced light colors
  vec3 hor_col = HORIZON_BASE_COLOR;
  vec3 sun_col = vec3(0.9, 0.8, 0.7);
  vec3 bou_col = BOUNCED_LIGHT_COLOR;
  ...
  // Use POST_SATURATION parameter
  col = colorgrade_colorgrade_saturate(col, POST_SATURATION);
  
  // --- Apply Post-processing Brightness and Contrast ---
  // 1. Brightness
  col += POST_BRIGHTNESS;

  // 2. Contrast (pivot around 0.5)
  col = mix(vec3(0.5), col, POST_CONTRAST);
  
  // 3. Clamp Brightness Output (optional, but good for preventing blown out highlights)
  col = min(col, vec3(POST_CLAMP_BRIGHTNESS_OUTPUT));
  // --- End Post-processing Brightness and Contrast ---
