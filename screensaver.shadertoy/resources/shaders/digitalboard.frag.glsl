...
// vertical segment with the bottom of the segment being s and having length d
...
// horizontal segment with the left of the segment being s and having length d
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    float display_width = 1010.0;
    float cc = floor(display_width / (g_cw * (1.0 + g_cwb))); // character count per line

    vec2 uv = fragCoord / iResolution.x;
    uv.y = iResolution.y / iResolution.x - uv.y; // type from top to bottom, left to right
    uv *= display_width;

    int cs = int(floor(uv.x / (g_cw * (1.0 + g_cwb))) + cc * floor(uv.y / (g_ch * (1.0 + g_chb))));

    uv = mod_uv(uv);
    uv.y = g_ch * (1.0 + g_chb) - uv.y; // paint the character from the bottom left corner
    vec3 ccol = 0.35 * vec3(0.1, 0.3, 0.2) * max(smoothstep(3.0, 0.0, uv.x), smoothstep(5.0, 0.0, uv.y));
    uv -= vec2(g_cw * g_cwb * 0.5, g_ch * g_chb * 0.5);
    ...
    SP D R PE SP F O R R E S T E R SP A N D SP A G E N T SP F R A N K NL
    ...
    SP S O SP T H E Y SP C L O C K E D SP H I M SP O N SP T H E SP N O G G I N NL
    SP A N D SP S H O T SP H I M SP I N T O SP S P A C E SP PE PE PE

    float throb = 0.4 + 0.2 * (0.5 * sin(4.0 * iTime - 0.05 * float(cs)) + 0.5);
    vec3 tcol = vec3(0.7, 1.0, 0.8) * smoothstep(0.2, 0.0, tx);
    vec3 tglow = vec3(0.7, 1.0, 0.8) * (0.3 * throb + 0.7 * mix(0.6, 1.0, throb) * smoothstep(throb, 0.1, tx));

    vec3 terminal_color = ccol + tcol + tglow;
		// Test this one.


	vec3 bgcolor = 0.35 * vec3(0.1, 0.3, 0.2) * max(smoothstep(3.0, 0.0, uv.x), smoothstep(5.0, 0.0, uv.y));

    float on_duration = 12.0;
    float off_duration = 4.0;
    float fade_duration = 2.0;

    float cycle_duration = on_duration + off_duration + 2.0 * fade_duration;
    float current_time = mod(iTime, cycle_duration);
    float factor = 0.0;

    if (current_time < fade_duration) {
        factor = current_time / fade_duration; // Fade In
    } else if (current_time < fade_duration + on_duration) {
        factor = 1.0; // On
    } else if (current_time < fade_duration + on_duration + fade_duration) {
        float fade_progress = current_time - (fade_duration + on_duration);
        factor = 1.0 - (fade_progress / fade_duration); // Fade Out
    } else {
        factor = 0.0; // Off
    }

    vec3 final_color = mix(bgcolor, terminal_color.rgb, factor);
