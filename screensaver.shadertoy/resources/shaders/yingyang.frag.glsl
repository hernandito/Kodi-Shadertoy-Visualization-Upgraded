...
const float zoomSpeed			= -0.05;	// how fast to zoom (negative to zoom out)
const float zoomScale			= 0.03;	// how much to multiply overall zoom (closer to zero zooms in)
...
const float ANIMATION_SPEED		= 0.30;	// global animation speed (1.0 = current speed, <1.0 slows, >1.0 speeds up)
...
{ 
    ...
    return theta + turnSpeed*s*iTime*ANIMATION_SPEED; 
}
...
void mainImage( out vec4 color, in vec2 uv )
{
    ...
    // wander center
    p.x += 0.5*sin(0.234*iTime*ANIMATION_SPEED);
    p.y += 0.5*sin(0.2*iTime*ANIMATION_SPEED);
    
    // time warp
    float time = iTime*ANIMATION_SPEED + timeWarp*length(p);
    ...
    // apply ivory white tint as a multiply blend
    vec3 ivoryColor = vec3(0.91, 0.827, 0.639); // Ivory white (adjust to fine-tune: increase/decrease RGB for warmer/cooler tones)
    color.rgb *= ivoryColor;
}