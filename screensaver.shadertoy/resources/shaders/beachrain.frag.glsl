...
//#define HAS_HEART
#define USE_POST_PROCESSING // <--- IMPORTANT: THIS LINE IS UNCOMMENTED for BCS to apply!

// --- New: Zoom Control ---
// Uncomment this line to disable the zoom effect and keep it zoomed out
#define DISABLE_ZOOM

// --- New: BCS Parameters ---
#define BRIGHTNESS 0.70 // Default brightness (1.0 = no change)
#define CONTRAST 1.00   // Default contrast (1.0 = no change)
#define SATURATION 1.0 // Default saturation (1.0 = no change)
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float zoom= mix(0.30, 1.2, story);   // slowly zoom out
    uv *=zoom;
    ...
    uv *= 1.5;                         // zoom out a bit more iTime
    t *= .25;
    #else
    float zoom;
    #ifdef DISABLE_ZOOM
        zoom = 1.0; // Fix zoom to the fully zoomed-out state
    #else
        zoom = -cos(T*.2); // Original oscillating zoom
    #endif
    
    uv *= .7+zoom*.3; // Apply zoom to the glass/raindrops layer
    #endif

    // Apply zoom to the background UV coordinates
    // When zoom is 1.0, this becomes (UV-.5)*1.0+.5, effectively no change to UV coords
    UV = (UV-.5)*(.9+zoom*.1)+.5; 
    
    float staticDrops = S(-.5, 1., rainAmount)*3.;
    ...
    vec3 col = textureLod(iChannel0, UV+n, focus).rgb;
        ...
        // --- Apply ONLY BCS (all other original post-processing removed) ---
