...
vec3 sphere(vec3 col, mat2 rot, vec3 bcol, vec2 p, float r, float aa) {
  ...
  if (z2 > 0.0) {
    ...
    // This line is now correct, using the overloaded tanh_approx function
    vec3 cspe = pow(max(dot(lightDir, cr), 0.0), 10.0)*tanh_approx(8.0*(bcol))*0.5;
