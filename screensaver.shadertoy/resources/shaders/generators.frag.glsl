...
// New define parameters for sphere color and reflectivity
#define SPHERE_COLOR vec3(1.5, 1.00, 0)  // Default grey color (RGB: 1, 1, 1)
#define SPHERE_REFLECTIVITY 1.50  // Default reflectivity strength
...
vec3 light(in vec3 p, in vec3 dir, in vec3 n, in float hid) {
    ...
    float y = 3.35 - p.y;
    vec3 amb = max(.5, dot(dir, -n)) * 0.5 * AMBIENT_COLOR;
    if (hid < 0.5) {
        amb += max(0.2, dot(vec3(0., 1., 0.), -n)) * FLOOR_COLOR * pow(max(0., 0.2 - abs(3. - p.y)) / 0.2, 1.5) * 2.;
        amb += energy * pow(max(0., 0.4 - abs(y)) / 0.4, 2.) * max(0.2, dot(vec3(0., -sign(y), 0.), -n)) * 2.;
    }
    vec3 r = reflect(lightdir, n);
    float spec = pow(max(0., dot(dir, -r)) * sh, 10. * SPHERE_REFLECTIVITY);  // Apply reflectivity strength
    vec3 col;
    float energysource = pow(max(0., 0.04 - abs(y)) / 0.04, 4.) * 2.;
    if (hid > 1.5) {  // Sphere condition
        col = SPHERE_COLOR;  // Use defined sphere color
        spec = spec * spec;
    } else {
        float k = _texture(p) * 0.23 + 0.2;
        k = min(k, 1.5 - energysource);
        col = mix(vec3(k, k * k, k * k * k), vec3(k), 0.3);
        if (abs(hid - 1.) < 0.001) col *= FLOOR_COLOR * 1.3;
    }
    col = col * (amb + diff * LIGHT_COLOR) + spec * LIGHT_COLOR;
    if (hid < 0.5) {
        col = max(col, energy * 2. * energysource);
    }
    col *= min(1., ao + length(energy) * 0.5 * max(0., 0.1 - abs(y)) / 0.1);
    return col;
}

vec3 raymarch(in vec3 from, in vec3 dir) {
    float ey = mod(t * 0.5, 1.);
    ...
    for (int i = 0; i < RAY_STEPS; i++) {
        if (d.x > det && totdist < 3.0) {
            ...
            totdist += d.x;
            energy = ENERGY_COLOR * (1.5 + sin(iTime * 20. + p.z * 10.)) * 0.25;
            if (d.x < 0.015) glow += max(0., 0.015 - d.x) * exp(-totdist);
            if (d.y < 0.5 && d.x < 0.03) { // ONLY DOING THE GLOW WHEN IT IS CLOSE ENOUGH
                ...
                eglow += max(0., 0.03 - d.x) / 0.03 *
                    (pow(max(0., 0.05 - glw) / 0.05, 5.) +
    ...
    vec3 lglow = LIGHT_COLOR * pow(l, 30.) * 0.5;
    col += glow * (backg + lglow) * 1.3;
    col += pow(eglow, 2.) * energy * 0.015;
    col += lglow * min(1., totdist * totdist * 0.3);
    if (ref > 0.5) {
        vec3 sphlight = light(origfrom + sphdist * origdir, origdir, sphNorm, 2.);
        col = mix(col * 0.3 + sphlight * 0.7, backg, 1.0 - exp(-1. * pow(sphdist, 1.5)));
    }
    return col;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    pth1 = path(t + 0.3) + origin + vec3(0., 0.01, 0.);
    ...
    color = mix(rain, color, clamp(iTime * 0.5 - 0.5, 0., 1.));
    color *= 1. - pow(length(uv2 * uv2 * uv2 * uv2) * 1.1, 6.);
    uv2.y *= iResolution.y / 360.0;
    color.r *= (0.5 + abs(0.5 - mod(uv2.y, 0.010) / 0.010) * 0.5) * 1.5;  // Changed 0.021 to 0.010
    color.g *= (0.5 + abs(0.5 - mod(uv2.y + 0.007, 0.010) / 0.010) * 0.5) * 1.5;  // Changed 0.021 to 0.010
    color.b *= (0.5 + abs(0.5 - mod(uv2.y + 0.014, 0.010) / 0.010) * 0.5) * 1.5;  // Changed 0.021 to 0.010
    color *= 0.9 + rain * 0.35;
    #endif
