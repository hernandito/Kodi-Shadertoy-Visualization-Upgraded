...
#define GLOBAL_ANIMATION_SPEED 0.40 // Adjusts the overall speed of the animation (1.0 = normal speed)

// Brightness, Contrast, Saturation (BCS) Parameters
#define BRIGHTNESS_ADJUST 0.0      // Brightness adjustment (-1.0 to 1.0, 0.0 = no change)
#define CONTRAST_ADJUST   1.003      // Contrast adjustment (0.0 to inf, 1.0 = no change)
#define SATURATION_ADJUST 1.0      // Saturation adjustment (0.0 to inf, 1.0 = no change)
#define CLAMP_BRIGHTNESS_OUTPUT 1.0 // Clamps the brightest output color (1.0 = no clamp, <1.0 clamps values)
// --- END NEW GLOBAL PARAMETERS ---
...
vec4 makeSphere( vec2 pos )
{
    ...
    // Apply GLOBAL_ANIMATION_SPEED to iTime
    ra *= 0.85+0.15*sin( 1.5*(iTime * GLOBAL_ANIMATION_SPEED) + hash1(pos)*130.0 );
    
    // Apply GLOBAL_ANIMATION_SPEED to iTime
    ce.y += 0.3*smoothstep( 0.995, 0.996, sin(0.015*(iTime * GLOBAL_ANIMATION_SPEED)+100.0*hash1(hash1(pos))) );
...
vec3 makeEmission( in vec2 p )
{
    ...
    // Apply GLOBAL_ANIMATION_SPEED to iTime
    return mate * smoothstep( 0.995, 0.998, sin(0.015*(iTime * GLOBAL_ANIMATION_SPEED)+100.0*hash1(id)) );
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    int a = 0;
    {
        ...
        // Apply GLOBAL_ANIMATION_SPEED to iTime
        float time = 0.3*(iTime * GLOBAL_ANIMATION_SPEED) + 50.0*mo.x;
    #else
    for( int a=0; a<VIS_SAMPLES; a++ )
    {
        vec4 rr = fract( vec4(10.5421,22.61129,30.7123,35.36291) * dot(vec2(0.0149451,0.038921),fragCoord+float(a)*vec2(21,47)));
                            
        vec2 p = -1.0 + 2.0*(fragCoord.xy+rr.xz) / iResolution.xy;
        p.x *= iResolution.x/ iResolution.y;
        // Apply GLOBAL_ANIMATION_SPEED to iTime
        float time = 0.3*((iTime * GLOBAL_ANIMATION_SPEED) + 1.0*(0.5/24.0)*rr.w) + 50.0*mo.x;
    #endif    
    ...
    // 1. Brightness
    tot += BRIGHTNESS_ADJUST;

    // 2. Contrast (pivot around 0.5)
    tot = mix(vec3(0.5), tot, CONTRAST_ADJUST);

    // 3. Saturation
    float luma = dot(tot, vec3(0.2126, 0.7152, 0.0722)); // Standard sRGB luminance
    tot = mix(vec3(luma), tot, SATURATION_ADJUST);

    // 4. Clamp Brightness Output (optional, but good for preventing blown out highlights)
    tot = min(tot, vec3(CLAMP_BRIGHTNESS_OUTPUT));
    // --- END BCS ---

    tot = pow( clamp(tot,0.0,1.0), vec3(0.44) ); // Original gamma correction and clamp
