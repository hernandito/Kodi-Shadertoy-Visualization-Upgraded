...
{
	return texture( iChannel0, (vec2(x)+0.5)/356.0, -100.0 );
}

vec4 Rand( in int x )
{
	vec2 uv;
	uv.x = (float(x)+0.5)/456.0;
	uv.y = (floor(uv.x)+0.5)/456.0;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	for ( int i=0; i < 6; i++ )
	{
		...
		float w = pow(max(0.0,1.0-18.0*length(fract(pos.xy)-.5)),2.0);
