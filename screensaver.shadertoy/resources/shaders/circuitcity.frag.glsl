...
#define time iTime*.004  // Keep the original time scale

// Define BCS parameters
#define BRIGHTNESS 0.90  // Default brightness (1.0 is neutral)
#define CONTRAST 1.2    // Default contrast (1.0 is neutral)
#define SATURATION 1.1  // Default saturation (1.0 is neutral)
...
void formula(vec2 z, float c) {
    ...
    vec3 col=normalize(.1+texture(iChannel0,vec2(minit*.1)).rgb);  // Moved image to iChannel0
    color+=col*(.4+mod(minit/9.-time*10.+ot2*2.,1.)*1.6);
    // Replace audio-dependent smoothstep with a static or time-based gold contribution
    color+=vec3(1., .7, .3)*circ*(10.-minit)*3.*smoothstep(0., .5, .15 + sin(time*2.)*.5);  // Static time-based modulation
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Brightness
    colo = colo * BRIGHTNESS;
    // Contrast
    colo = (colo - 0.5) * CONTRAST + 0.5;
    // Saturation
    float luminance = dot(colo, vec3(0.299, 0.587, 0.114));
    colo = mix(vec3(luminance), colo, SATURATION);
