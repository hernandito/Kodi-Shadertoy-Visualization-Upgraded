...
const vec3 LC = vec3(0.859, 0.733, 0.498);    // light colour
...
Camera getCamera(in vec2 uv, in vec3 pos, in vec3 target) {
    ...
    float FOV = 1.+cos(iTime*.05)*.8; // Slowed down from iTime*.1 to iTime*.05
...
Model model(vec3 p) {
    float t = iTime*.05; // Slowed down from iTime*.1 to iTime*.05
    p.xz *= R(t);
    p.xy *= R(.3);
    p.xy -= .5;
