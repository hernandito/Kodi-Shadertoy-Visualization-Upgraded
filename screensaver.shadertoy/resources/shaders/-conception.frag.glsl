...
float noise( in vec3 x )
{
    ...
    // Use vec2 indexing instead of texelFetch for Kodi compatibility
    vec2 uv = (p.xy + vec2(37.0,17.0)*p.z);
    uv = mod(uv, 256.0); // wrap in 0–255 range
    vec2 rg1 = texture2D(iChannel0, (uv + vec2(0.5)) / 256.0).yx;
    vec2 rg2 = texture2D(iChannel0, (uv + vec2(1.5, 0.5)) / 256.0).yx;
    vec2 rg3 = texture2D(iChannel0, (uv + vec2(0.5, 1.5)) / 256.0).yx;
    vec2 rg4 = texture2D(iChannel0, (uv + vec2(1.5, 1.5)) / 256.0).yx;

    vec2 rgx = mix(rg1, rg2, f.x);
    vec2 rgy = mix(rg3, rg4, f.x);
    vec2 rg  = mix(rgx, rgy, f.y);
