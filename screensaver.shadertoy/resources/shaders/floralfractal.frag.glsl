...
// Add precision declaration, often required by GLSL ES 1.0 compilers.
...
#define CONTRAST   1.30 // 0.0 to inf, 1.0 is no change
#define SATURATION 0.90 // 0.0 to inf, 1.0 is no change
// --- End BCS Parameters ---

// --- Screen Offset Parameters ---
// These values will shift the center of the effect.
// Positive X moves the center to the right, positive Y moves it up.
// Adjust these values (e.g., -0.5 to 0.5) to move the effect.
#define OFFSET_X -0.20
#define OFFSET_Y -0.125
// --- End Screen Offset Parameters ---

// --- Rotation Parameter ---
// Defines the speed of the screen rotation.
// Positive values for clockwise rotation.
// A very small value like 0.05 will result in a slow rotation.
#define ROTATION_SPEED 0.05 // Adjust as needed for desired speed
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Store the original offset values for rotation
    vec2 offset = vec2(OFFSET_X, OFFSET_Y);

    // Apply screen offset
    p += offset; // Apply offset here initially

    // Apply rotation around the new center (offset point)
    float angle = time * ROTATION_SPEED; // Calculate rotation angle
    mat2 rotMatrix = mm2(-angle); // Clockwise rotation, so negative angle for standard rot2 matrix
    
    // Translate p back to origin, rotate, then translate back by offset
    p = (p - offset) * rotMatrix + offset;
