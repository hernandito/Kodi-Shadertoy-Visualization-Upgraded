...
#define VIEW_SCALE 1.200 // Base view scale (default 0.8, range: 0.1 to 2.0 for zoom out/in)
#define ANIM_SPEED 0.60 // Overall animation speed (default 1.0, range: 0.0 to 2.0 for stopped to double speed)
#define BC vec3(0.859, 0.431, 0.188) // Base color for the palette (default blue, adjust to change the entire color scheme)
...
void sphere (vec4 hx, vec2 st, float sm, inout vec4 R) //memory sphere full of data
{
    ...
    float T = mod(iTime * ANIM_SPEED + h21(hx.zw * 20.) * 20., 20.) //local time
        ...
        , y = .4 * ((T < 4.) ? sin((T - 1.) * .52) : //sphere position
                    ...
                    ((T < 8.5) ? sin((8.5 - T) * .52) : 
                    0.))) - .06          
        , f = (.9 + noise(vec2(hx.x * 50. + iTime * ANIM_SPEED * 4.)) * .3) //hole inner noise
    ...
    f = noise(hx.xy * vec2(12, 7) + vec2(0, iTime * ANIM_SPEED * -4.)) * .25 + .5; //sphere noise
...
void pixel (float hh, float sm, vec2 st, vec2 s, float n, vec4 R, inout vec4 C) //blinking pixels
{
    ...
    float b = ((4. - n) * 2.2 + .8) * .05 //face lightness
        ...
        , T = mod(iTime * ANIM_SPEED + hh * 20., 20.) //local timer
        , d = ((T < 3.) ? sin((T) * .52) : //hole diameter
                        ((T < 6.5) ? 1. :   
                        ((T < 9.5) ? sin((9.5 - T) * .52) : 
                                0.)))
        , f = min(
                (pow(noise(id * hh * n + iTime * ANIM_SPEED * (.75 + h21(id) * .15) * 1.), 8.) * 2. //small picks
                + (noise(id * .2 + iTime * ANIM_SPEED * (.5 + hh * n) * .5) - .1)) //big noise
...
{
    vec4 hx = getHex(uv);
    vec2 s; //top face square coordinates 
...
void mainImage(out vec4 C, in vec2 g)
{
    ...
    uv *= VIEW_SCALE + sin(iTime * ANIM_SPEED * .3) * .25; //camera scale
    uv -= uv * pow(length(uv), 2.5 - sin(iTime * ANIM_SPEED * .3) * .5) * .025 +  //camera distortion
        vec2(iTime * ANIM_SPEED * .2, cos(iTime * ANIM_SPEED * .2));   //camera translate
