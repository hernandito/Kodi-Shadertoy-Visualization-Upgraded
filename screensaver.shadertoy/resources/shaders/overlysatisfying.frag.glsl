...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	planet_cracks = (abs(sin(t * .0)) * 0.002 + 0.0007) * planet_cracks;
...
vec3 calc_pal(float x)
{
	vec3 col = mix(vec3(0.8,0.72,0.16), vec3(0.64,0.16,0.08), smoothstep(1.2, 1.6, x));
	col = mix(col, vec3(0.0, 0.0, 0.0), smoothstep(1.5, 1.8, x*(1.1)));
	return col;
}
...
// Mix final noise value
  ...
  m = m * m;
  return 18.0 * dot( m*m, vec4( dot(p0,x0), dot(p1,x1),
                                dot(p2,x2), dot(p3,x3) ) );
}