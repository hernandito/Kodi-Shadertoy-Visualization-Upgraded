...
#define BRIGHTNESS 1.0  // Default brightness (1.0 is no change)
#define CONTRAST 1.4    // Default contrast (1.0 is no change)
...
// colors
vec3 col_water = vec3(0.467, 0.863, 0.902);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  col = mix(vec3(0.5), col, CONTRAST);  // Contrast
  col = col * BRIGHTNESS;               // Brightness
  float luminance = dot(col, vec3(0.299, 0.587, 0.114));
  col = mix(vec3(luminance), col, SATURATION);  // Saturation
