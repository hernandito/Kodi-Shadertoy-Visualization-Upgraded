...
vec2 hash22(vec2 p) { 
    ...
    return sin(p*6.2831853 + iTime*ANIMATION_SPEED*2.);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float c = Voronoi3Tap(uv*CELL_SCALE);
    float c2 = Voronoi3Tap(uv*CELL_SCALE - 9.*OUTLINE_THICKNESS/iResolution.y);
    ...
    col = mix(col, col2, smoothstep(.4, .6, sin(iTime*ANIMATION_SPEED*.25)*.5 + .5)); // 
