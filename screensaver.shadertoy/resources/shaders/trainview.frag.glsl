...
vec3 ground(in vec2 u, in vec3 c, in float shadow_pos)
{
    if(u.y<0.4)
    {
        const float b_crisp = 0.0001; // Very small blur for crisp edges of main shapes
        const float b_shadow = 0.005; // Original blur value for shadows

        vec2 op = u*2.0;
        op.x += iTime*0.05;
        c=mix(c, txc(c0, 0.98, vec2(2.5,2.5)), smoothstep(b_crisp*5.0, -b_crisp*5.0, GRND5));

        op = vec2(u.x*3.0 + iTime*0.1 - shadow_pos, u.y*3.0-0.5);
        // Apply b_shadow for this shadow layer
        c=mix(c, c*0.75, smoothstep(b_shadow*30.0, -b_shadow*30.0, GRND4));
        op.x += shadow_pos;
        c=mix(c, txc(c1, 0.98, vec2(1.33,1.33)), smoothstep(b_crisp*3.0, -b_crisp*3.0, GRND4));

        op = vec2(u.x*4.0 + iTime*0.2 - shadow_pos, u.y*3.0-0.2);
        // Apply b_shadow for this shadow layer
        c=mix(c, c*0.9, smoothstep(b_shadow*10.0, -b_shadow*10.0, GRND3));
        op.x += shadow_pos;
        c=mix(c, txc(c2, 0.98, vec2(0.75, 1.0)), smoothstep(b_crisp*0.5, -b_crisp*0.5, GRND3));
        
        op = vec2(u.x*5.0 + iTime*0.4 - shadow_pos, u.y*2.0);
        // Apply b_shadow for this shadow layer
        c=mix(c, c*0.82, smoothstep(b_shadow*20.0, -b_shadow*20.0, GRND2));
        op.x += shadow_pos;
        c=mix(c, txc(c3, 0.98, vec2(0.4,1.0)), smoothstep(b_crisp*3.0, -b_crisp*3.0, GRND2));
        
        op = vec2(u.x*8.0 + iTime -shadow_pos, u.y*2.0+0.02);
        // Apply b_shadow for this shadow layer
        c=mix(c, c*0.75, smoothstep(b_shadow*30.0, -b_shadow*30.0, GRND1));
        op += vec2(shadow_pos, -0.02);
        c=mix(c, txc(c4, 0.96, vec2(0.5, 1.0)), smoothstep(b_crisp*5.0, -b_crisp*5.0, GRND1));        
    }    
    return c;
}
...
vec3 cloud(in vec2 u, in vec2 p, in float iscale,in vec3 c, const in vec3 cloud_color, in vec2 shadow_pos, in float shadow_factor, in float blur, in float shadow_blur)
{
    ...
    d = min(d, length((st  +vec2(0.06, 0))) - 0.055);
    vec2 op = st;
    // Set blur to a very small value for crisp edges
    c=mix(c, cloud_color*0.98 + (1.0-0.98)*texture(iChannel0, st*2.5).r, smoothstep(0.0001, -0.0001, d));

    // thread
    // Set blur to a very small value for crisp edges
    c=mix(c, cloud_color*0.65, smoothstep(0.0001 / (iscale*iscale), -(0.0001*0.5)/(iscale*iscale), sdLine(u, p + vec2(0,0.065), vec2(p.x, iscale))));
    return c;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    c = cloud(uv, np, 2.0, c, c7, vec2(shadow_pos, -0.1)*0.2, 0.8,  0.0001, 0.03);
    ...
    c = cloud(uv, np, 1.5, c, c6, vec2(shadow_pos, -0.1)*0.2, 0.8,  0.0001, 0.04);
    ...
    c = cloud(uv , np, 1.0, c, c5, vec2(shadow_pos, -0.1)*0.2, 0.8,  0.0001, 0.04);
