...
#define DAY_CYCLE_DURATION 28.0

// Define start and end of the day cycle (0.0 to 1.0, where 0.0 is sunrise, 1.0 is sunset)
#define DAY_START 0.0 // 7 AM
#define DAY_END 0.99 // 5 PM

// Post-processing parameters (adjust these values)
#define BRIGHTNESS 1.1 // 1.0 is no change
#define CONTRAST 1.55  // 1.0 is no change
#define SATURATION 1.40 // 1.0 is no change
...
vec3 sunDir(float day_progress_0_1) {
    // This creates a linear movement of the sun horizontally
    // and a smooth arc vertically to avoid the speed variation.
    float azimuth_angle = mix(-1.5, 1.5, day_progress_0_1); // Linear horizontal sweep
    float elevation_angle = sin(day_progress_0_1 * 3.141592) * 0.8 + 0.2; // Smooth vertical arc
    
    // Combine the linear horizontal movement with the curved vertical path
    return normalize(vec3(sin(azimuth_angle), elevation_angle, cos(azimuth_angle)));
}

vec3 sunColor(float day_progress_0_1) {
    float daylightFac = sin(day_progress_0_1 * 3.141592);
    vec3 dawnDuskColor = vec3(1.0, 0.4, 0.13);
    vec3 middayColor = vec3(0.95, 0.91, 0.77);
    return mix(dawnDuskColor, middayColor, daylightFac) * 4.0; // Increased sun brightness to 4.0
}

vec3 day(vec3 dir, float day_progress_0_1) {
    float daylightFac = pow(cos(day_progress_0_1 * 3.141592 + sin(day_progress_0_1 * 3.141592) / 3.0) / 2.0 + 0.5, 2.0);
    return mix(vec3(0.0, 0.1, 0.2), vec3(0.4, 1.0, 0.9) * 2.0, min(airMass(dir) / 11.0, 1.0)) * daylightFac;
}
...
vec3 ambientColor(float day_progress_0_1) {
    float daylightFac = sin(day_progress_0_1 * 3.141592);
    return mix(vec3(0.2, 0.3, 0.4), vec3(0.6, 0.8, 0.9), daylightFac);
}
...
vec4 renderScene(in vec2 fragCoord_in, float day_progress_0_1, float visibility_alpha) {
    ...
    if (res.nearest.water) {
        ...
        vec3 reflected_color = shade_overload(refl, day_progress_0_1);
        vec3 refracted_color = shade_overload(refr, day_progress_0_1) * refrColor;
        res.color = mix(refracted_color, reflected_color, reflFactor);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    float fade_in_duration = 1.0;
    float day_duration = DAY_CYCLE_DURATION;
    float fade_out_duration = 1.0;
    float total_cycle_duration = fade_in_duration + day_duration + fade_out_duration;
    ...
    if (cycle_time < fade_in_duration) {
        visibility_alpha = smoothstep(0.0, 1.0, cycle_time / fade_in_duration);
        day_progress_0_1 = DAY_START;
    } else if (cycle_time < fade_in_duration + day_duration) {
        day_progress_0_1 = (cycle_time - fade_in_duration) / day_duration * (DAY_END - DAY_START);
        visibility_alpha = 1.0;
    } else { // fade out period
        visibility_alpha = smoothstep(1.0, 0.0, (cycle_time - (fade_in_duration + day_duration)) / fade_out_duration);
        day_progress_0_1 = DAY_END;
    }
    
    #if AA == 1
    vec4 finalColor = renderScene(fragCoord, day_progress_0_1, visibility_alpha);
    #else
    vec4 o = vec4(0.0);
    ...
    vec4 finalColor = o / max(float(AA), TINY_EPSILON);
    #endif

    // Apply post-processing
    vec3 post_color = finalColor.rgb;

    // Brightness
    post_color *= BRIGHTNESS;

    // Contrast
    post_color = (post_color - 0.5) * CONTRAST + 0.5;

    // Saturation
    vec3 luma = vec3(0.299, 0.587, 0.114);
    vec3 gray = vec3(dot(post_color, luma));
    post_color = mix(gray, post_color, SATURATION);

    fragColor = vec4(post_color, finalColor.a);
}