...
vec3 render(vec3 ro, vec3 rd) {
  ...
  for (int bounce = 0; bounce < MAX_BOUNCES; ++bounce) {
    ...
    // Edge-cropping anti-aliasing: Fast fall-off to zero transparency near edges
    float edgeThreshold = 0.002; // Distance threshold for edge detection
    if (abs(df(sp)) < edgeThreshold || float(iter) > float(MAX_RAY_MARCHES) * 0.02) { // Cast iter to float
      ragg *= smoothstep(edgeThreshold, 0.0, abs(df(sp))); // Rapid fall-off to 0
    }
...
vec3 effect(vec2 p) {
  ...
  vec3 ro = 0.6 * vec3(0.0, 2.0, 7.50); // Camera position (ro), scaled by 0.6
  // Adjust the z-value in vec3(0.0, 2.0, z_value) to pull the camera back or forward:
  // - Increase z_value (e.g., 15.0) to pull the camera back further, showing more of the cube.
  // - Decrease z_value (e.g., 5.0) to bring the camera closer, showing less.
  // Current z_value is 10.0, resulting in a distance of ~6.12 units from the origin.
