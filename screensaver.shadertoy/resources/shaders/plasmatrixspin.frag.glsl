...
vec4 slice(float x0, float x1, vec2 uv)
{
    ...
    //Lighting 
    col *= w / sqrt(8.0 * IN_RADIUS*IN_RADIUS * (1.0 - cos(tau / float(NUM_FACES))));
    
    //Edges
    col *= smoothstep(0.05, 0.40, u) * smoothstep(0.95, 0.90, u) + 0.5;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    //Twisting angle
    float angle = uvr.y + .20*iTime + sin(uvr.y) * sin(iTime) * pi;
