...
// MATERIAL_RED_COLOR: Diffuse color for the first material (red-ish)
#define MATERIAL_RED_COLOR vec3(0.62, 0, 0)
// MATERIAL_BLUE_COLOR: Diffuse color for the second material (blue-ish)
#define MATERIAL_BLUE_COLOR vec3(1, 0.961, 0.894)*2.4
// ---------------------------------

// --- Light Color Parameters ---
// LIGHT4_CLAMP_VALUE: Maximum value for the white light's color components (0.9 for clamping)
#define LIGHT4_CLAMP_VALUE 0.95
// ------------------------------
...
vec3 l1color = 40.*vec3(1, 0.961, 0.894);
vec3 l2color = 20.*vec3(.7);
vec3 l3color = 60.*vec3(.7);
...
mat_t material(vec3 p) {
    ...
    m.specular = vec4(0.);
    if (w1(p) < w2(p)) {
        m.diffuse = MATERIAL_RED_COLOR; // Using the new define
        m.specular = vec4(mix(vec3(1.),m.diffuse,.7), 2000.);
    } else {
        m.diffuse = MATERIAL_BLUE_COLOR; // Using the new define
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    // Apply clamp to the final color (post-processing)
    color = clamp(color, 0.0, LIGHT4_CLAMP_VALUE); // Apply clamp here
