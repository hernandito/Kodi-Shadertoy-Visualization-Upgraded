...
#define FOV_ADJUSTMENT      0.90       // Adjusts the field of view (1.0 is default).
...
vec2 hextile(inout vec2 p) {
  ...
  // GLSL 1.10 does not have round(), so we use a simple floor-based approximation.
...
vec3 effect(vec2 p) {
  const vec3 ro = .66*vec3(5.0, -1.0, 1.3);
//  const vec3 ro = 0.8*vec3(5.0, 0.0, 0.0);
  ...
  const float fov = tan(TAU/6.0) * FOV_ADJUSTMENT;
