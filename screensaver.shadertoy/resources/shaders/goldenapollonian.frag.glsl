...
// A parameter to control the overall animation speed.
// 1.0 is default speed, 2.0 is double speed, 0.5 is half speed, etc.
#define ANIM_SPEED_SCALE 2.0

// Custom round function, as the built-in round() is not available in GLSL ES 1.00
// It has been renamed to avoid a conflict on the website's compiler.
...
// Struct to hold effect parameters
struct effect {
    float lw;
    float tw;
    float sk;
...
// Function to select and return an effect struct
effect getEffect(int index) {
    if (index == 0) {
        return effect(0.125, 0.0, 0.0, 0.0);
    } else if (index == 1) {
        return effect(0.125, 0.0, 0.0, 1.0);
    } else if (index == 2) {
        return effect(0.125, 0.0, 1.0, 1.0);
    } else if (index == 3) {
        return effect(0.125, 1.0, 1.0, 1.0);
    } else if (index == 4) {
        return effect(0.125, 1.0, 1.0, 0.0);
    }
    // Default to a specific effect if index is out of bounds
    return effect(0.125, 1.0, 0.0, 0.0);
}
...
float apollian(vec4 p, float s, effect ceffect) {
    float scale = 1.0;
    for(int i=0; i<7; ++i) {
...
float weird(vec2 p, float h, effect ceffect) {
    float z = 4.0;
    // Apply the speed scale to the animation time for this effect
    float tm = (0.1*TIME+h*10.0) * ANIM_SPEED_SCALE;
    p *= ROT(tm*0.5);
    float r = 0.5;
    ...
    pp /= z;
    float d = apollian(pp, 0.8+h, ceffect);
    return d*z;
}
...
vec2 df(vec2 p, float h, effect ceffect) {
    vec2 wp = p;
    float rep = 2.0*myRound(mix(5.0, 15.0, h*h));
...
vec2 df(vec3 p, vec3 off, float s, mat2 rot, float h, effect ceffect) {
    vec2 p2 = p.xy;
    p2 -= off.xy;
    p2 *= rot;
    return df(p2/s, h, ceffect)*s;
}
...
vec4 plane(vec3 ro, vec3 rd, vec3 pp, float pd, vec3 off, float aa, effect ceffect, float n) {
    float h = hash(n);
    ...
    vec2 d2 = df(pp, off, s, rot, h, ceffect);
    ...
    vec2 bd  = df(bpp, off, s, rot, h, ceffect);
    vec2 sd1 = df(spp1, off, s, rot, h, ceffect);
    vec2 sd2 = df(spp2, off, s, rot, h, ceffect);
...
vec3 color(vec3 ww, vec3 uu, vec3 vv, vec3 ro, vec2 p) {
    ...
    float rdd = (2.0-0.5*tanh_approx(lp));
    ...
    for (int i = 1; i <= furthest; ++i) {
        ...
        if (pd > 0.0 && acol.w < cutOff) {
            ...
            int pi = int(mod(myRound((nz+float(i))/PLANE_PERIOD), 6.0));
            effect current_effect = getEffect(pi);

            vec3 off = offset(pp.z);

            vec4 pcol = plane(ro, rd, pp, pd, off, aa, current_effect, nz+float(i));
...
vec3 color(vec2 p, vec2 q) {
    // Apply the speed scale to the animation time
    float tm  = (TIME*0.125) * ANIM_SPEED_SCALE;
    vec3 ro   = offset(tm);
    vec3 dro  = doffset(tm);
