...
vec3 raytraceRay( vec3 ro, vec3 rd, inout Inter i)
{
    ...
    // --- Floor Color Adjustment ---
    // Change the vec3 value to adjust the light grey floor color
    mat.color = vec3(0.4); // Light grey color
    // ------------------------------
    vec3 p = vec3(0.,0.,0.);
    vec3 n = vec3(0.,1.,0.);
    if (intPlane( ro, rd, p, n, mat, i))
    {
        // checker plane hack - removed for solid color
        // i.mat.color = vec3(0.75)*mod(floor(i.p.x)+floor(i.p.z),2.)+0.25;
    }
    //
    // --- Orange Wall Color Adjustment ---
    // Change the vec3 value to adjust the yellow wall color
    // Swapped color with spheres - now bright red
    mat.color = vec3(.20, 0.20, 0.20); // Bright red color (originally sphere color)
    // ------------------------------------
    p = vec3(-8.,0.,0.);
    n = vec3(-1.,0.,0.);
    if (intPlane( ro, rd, p, n, mat, i))
    ...
    //
    // --- Blue Wall Color Adjustment ---
    // Change the vec3 value to adjust the medium blue wall color
    mat.color = vec3(0.10, 0.1, 0.1); // Medium blue color
    // ----------------------------------
    p = vec3(0.,0.,8.);
    n = vec3(0.,0.,1.);
    if (intPlane( ro, rd, p, n, mat, i))
    ...
    //
    // --- Sphere Color Adjustment ---
    // Change the vec3 value to adjust the sphere color
    // Swapped color with orange wall - now slightly more orangish yellow
    mat.color = vec3(.40, 0.6, 0.0); // Slightly more orangish yellow color (originally orange wall color)
    // -------------------------------
    mat.reflection = false;
    mat.refraction = false;
    mat.n = 1.;
    ...
    //
    // --- Sphere Color Adjustment (Reflective Spheres) ---
    // This section also defines spheres; their color is set here
    // Swapped color with orange wall - now slightly more orangish yellow
    mat.color = vec3(1.0, 0.6, 0.0); // Slightly more orangish yellow color
    // ----------------------------------------------------
    mat.reflection = true;
    mat.refraction = false;
    mat.n = 1.;
    ...
    //
    // --- Sphere Color Adjustment (Refractive Spheres) ---
    // This section also defines spheres; their color is set here
    // Swapped color with orange wall - now slightly more orangish yellow
    mat.color = vec3(0.0, 0.0, 0.0); // Slightly more orangish yellow color
    // ----------------------------------------------------
    mat.reflection = false;
    mat.refraction = true;
    mat.n = 1.16;
    ...
    //
    // --- Sphere Color Adjustment (Center Sphere) ---
    // This section also defines spheres; their color is set here
    // Swapped color with orange wall - now slightly more orangish yellow
    mat.color = vec3(1.0, 0.8, 0.0); // Slightly more orangish yellow color
    // ---------------------------------------------
    mat.reflection = true;
    mat.refraction = true;
    mat.n = 1.33;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Decrease this value to slow down the animation
    // Increase this value to speed up the animation
    float animation_speed = 0.15; // Adjust this value for animation speed
    // ----------------------------------
    float Time = animation_speed*(15.0 + iTime);

    // camera
    vec3 ro = vec3( 8.0*cos(Time), 6.0, 8.0*sin(Time) );
//    vec3 ro = vec3( -8.0, 6.0, 8.0 ); // Original commented out camera position
    ...
    // --- FOV Adjustment ---
    // Increase the divisor value to narrow the FOV
    // Decrease the divisor value to widen the FOV
    float fov_factor = 1.5; // Adjust this value for FOV
    vec3 rd = normalize( p.x*cu + p.y*cv + fov_factor*cw );
    // ----------------------
