...
// Adjust this value to change the brightness of the metallic material
#define METALLIC_DARKENING 0.2
// Adjust this value to control the strength of the specular highlights (reflections)
#define SPECULAR_STRENGTH 0.40
...
const vec3  baseCol           = vec3(0.45, 0.45, 0.5)*0.5 * METALLIC_DARKENING;
...
vec3 render(vec3 ro, vec3 rd) {
  ...
  col += sunColor1*spe1*sha * SPECULAR_STRENGTH;
  col += sunColor2*spe2*sha * SPECULAR_STRENGTH;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // --- MODIFICATION: Continuous loop of the first scene only ---
