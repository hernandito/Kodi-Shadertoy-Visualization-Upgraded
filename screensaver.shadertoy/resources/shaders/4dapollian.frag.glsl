...
// New parameter to control global animation speed
#define SPEED_FACTOR 0.20

const int   max_iter    = 130;
const vec3  bone        = vec3(0.89, 0.839, 0.741);
...
float df(vec3 p) {  
  ...
  // Apply SPEED_FACTOR to the time variable
  float tm = SPEED_FACTOR * 0.1 * iTime;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )  { 
  ...
  // Apply SPEED_FACTOR to the camera rotation time
  rot(ro.xz, SPEED_FACTOR * 2.0*PI*iTime/120.0);
