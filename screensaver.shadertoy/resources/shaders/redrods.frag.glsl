// Day 57!
// A bit of inspiration taken from shadertoy.com/view/33syzM.

// =========================================================================
// USER-TUNABLE PARAMETERS
// =========================================================================
// Controls the density of the rods. Smaller value = Denser rods. (Default: 0.1)
#define ROD_CELL_SIZE 0.1

// The fixed color for the animating geometry (Dark Red: R, G, B components)
// Current value: vec3(0.4, 0.05, 0.05)
#define ROD_COLOR vec3(0.4, 0.05, 0.05)
...
// =========================================================================
#define OUTPUT_CLAMP_MAX (1.0) // Clamps the final bright colors to a maximum value.

#define res iResolution.xy

float gv;

// Utility function to generate a pseudo-random hash value from a 2D vector
float hash(vec2 p) {
...
// Scene SDF map for raymarching
float map(vec3 p) { 
    vec3 q = p;
    
    // Repetition technique for the rod grid
    float s = ROD_CELL_SIZE;
    vec2 cell = floor(q.xz / s);
    float d1 = 1e20; // Initialize minimum distance to a very high value

    // Check surrounding cells for rods
    for (int j = -2; j <= 2; ++j) {
        for (int i = -2; i <= 2; ++i) {
            ...
            float h = 0.4 + 0.9 * (0.5 + 0.5 * sin(iTime * 0.25 * 1.3 + seed * 6.282));

            // Rod geometry: Capped Cylinder with half-height h and radius h * 0.05
...
void mainImage(out vec4 O, in vec2 I)
{
    ...
    // Camera setup (moving backwards along Z)
    vec3 camPos = vec3(3.0, 3.0, -iTime * 0.1);
    vec3 camDir = normalize(vec3(0.0, -0.9, -1.0)); // Camera looking down and into the scene
    ...
    if (hit) {
        ...
        // Use the defined fixed color for the rods
        vec3 color = ROD_COLOR;
        
        // Exponential fog based on distance
        float dist = length(rayPos - camPos);
        float fog = min(pow(1.0 / dist * 4.0, 3.0), 1.0);
        
        // Approximate Ambient Occlusion (based on steps taken)
    ...
    // Apply final exposure boost (original * 3.0 multiplier)
    col *= 3.0;
